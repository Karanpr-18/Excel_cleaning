...
class ExcelValidator:
    def __init__(self):
        # Validation rules from the original script
    ...
    def get_max_marks(self, age, total=False):
        """Get maximum marks based on age"""
        try:
            age_num = int(age)
    ...
    def validate_excel(self, file_path, unique_id):
        """
        Validate Excel file and generate output files
        ...
        try:
            ...
            for col_name, rules in self.validation_rules.items():
                ...
                for i, value in enumerate(data_df[col_name], start=2):
                    ...
                    # Check age range for Student's Age
            ...
            output_path = os.path.join(app.config['DOWNLOAD_FOLDER'], f"{unique_id}_Validated_Output.xlsx")
            report_path = os.path.join(app.config['DOWNLOAD_FOLDER'], f"{unique_id}_Validation_Report.xlsx")
            ...
            logging.info(f"Validation complete. Generated {len(validation_errors)} error records.")
